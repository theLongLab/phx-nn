...
class HParamNet(BaseModel):
    def __init__(self, trial: Trial, hparam_space: Mapping[str, Sequence]) -> None:
        ...
        self.example_input_array = torch.rand(1, 45)
    ...
    def __optimize_layers(self, trial: Trial, hparam_space: Mapping[str, Sequence]) -> int:
        ...
        input_dim: int = 45  # number of input summary statistics
    ...
    def forward(self, *inputs):
        ...
        for layer, batchnorm, dropout in zip(self.layers, self.batchnorms, self.dropouts):
            x = F.relu(layer(x))
            x = batchnorm(x)
            x = dropout(x)
...
class LightningHParamNet(pl.LightningModule):
    def __init__(
        ...
        # Avoid overriding `Lightning.Module` attributes (e.g. self.model)
    ...
    def training_step(self, batch: Tuple, batch_idx: int) -> Dict[str, torch.Tensor]:
        ...
        output: torch.Tensor = self.forward(X)
    ...
    def validation_end(self, outputs: Sequence[Mapping]) -> Dict[str, Dict[str, float]]:
        ...
        mean_val_loss: torch.Tensor = sum(x["val_loss"].clone().detach() for x in outputs) / len(
            outputs
        )

        return {"log": {"val_loss": mean_val_loss.item()}}
    ...
    def test_end(self, outputs: Sequence[Mapping]) -> Dict[str, Dict[str, float]]:
        x: Dict[str, torch.Tensor]
        mean_test_loss: torch.Tensor = sum(x["test_loss"].clone().detach() for x in outputs) / len(
            outputs
        )
    ...
    def configure_optimizers(self) -> Optimizer:
        return AdaBound(self._model.parameters())
...
class PoolHapXNet(pl.LightningModule):
    ...
    def test_dataloader(self) -> DataLoader:
        return SumStatDataLoader(
            data=self._val_data, batch_size=512, shuffle=False, validation_split=0.0, num_workers=4,
        )
